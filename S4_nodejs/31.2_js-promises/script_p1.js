...
$("#q1").on("submit", (evt) => {
  ...
  axios
    .get(`${numApiUrl}${favNum}?json`)
    .then((resp) => $("#q1-resp").text(resp.data.text))
    .catch((err) => $("#q1-resp").text(err));
});
...
$("#q3").on("submit", (evt) => {
  ...
  let facts = "";
  Promise.all(fourFactProms)
    .then((respArr) => {
      // Basically the same as q2
      respArr.forEach((resp) => (facts += resp.data.text + "<br>"));
      $("#q3-resp").html(facts);
    })
    .catch((err) => $("#q3-resp").text(err));
});
